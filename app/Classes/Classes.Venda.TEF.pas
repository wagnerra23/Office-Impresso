...
type
  TStatusVenda = (stsLivre, stsIniciada, stsEmPagamento, stsCancelada, stsAguardandoTEF, stsOperacaoTEF, stsFinalizada, stsInativo);
  ...
  public
    ...
    property TipoPagamentoExtenso: String read FTipoPagamentoExtenso write FTipoPagamentoExtenso;
  ...
  function StatusVendaStrToEnum(AOperacao : String):TStatusVenda;
...
//  TStatusVenda = (stsLivre, stsIniciada, stsEmPagamento, stsCancelada, stsAguardandoTEF, stsOperacaoTEF, stsFinalizada);
  ...
  if AOperacao = 'CAIXA LIVRE' then
    result := stsLivre
  else if AOperacao = 'EM VENDA' then
    result  := stsIniciada
  else if AOperacao = 'EM PAGAMENTO' then
    result  := stsEmPagamento
  else if AOperacao = 'CANCELADA' then
    result  := stsCancelada
  else if AOperacao = 'TRANSACAO TEF' then
    result  := stsAguardandoTEF
  else if AOperacao = 'OPERAÇÃO TEF' then
    result  := stsOperacaoTEF
  else if AOperacao = 'FINALIZADA' then
    result  := stsFinalizada
  else         // AOperacao = 'CAIXA LIVRE' then
    result := stsLivre ;
end;

//function StatusVendaStrToEnum(var ok: boolean; const s: string): TStatusVenda;
//begin
//  ok := False;
//  for Result := Low(TStatusVenda) to High(TStatusVenda) do
//  begin
//    if StatusVendaToStr(Result) = s then
//    begin
//      ok := True;
//      Exit;
//    end;
...
var
  ...
  i: Integer;
begin
  Ini := TIniFile.Create(FArqVenda);
  try
//    Ini.CleanupInstance;// Clear;
    Ini.WriteString('Venda','CodVenda', FCodVenda);
...
begin
  ...
  Ini := TIniFile.Create(FArqVenda);
  try
    FCodVenda := Ini.ReadString('Venda','CodVenda', '');
