unit Frame_CadProduto_UsoEConsumo;
...
type
  TFrmProduto_UsoEConsumo_Frame = class(TFrame_Produto_Mestre)
...
var
  FrmProduto_UsoEConsumo_Frame: TFrmProduto_UsoEConsumo_Frame;
...
procedure TFrmProduto_UsoEConsumo_Frame.Produto_TipoBeforeOpen(
  DataSet: TDataSet);
begin
  inherited;
  ...
  Produto_Tipo.SQL.Add('(CLASSIFICACAO = ''USOECONSUMO'')');
  Produto_Tipo.SQL.Add('order by DESCRICAO asc');
end;

procedure TFrmProduto_UsoEConsumo_Frame.WREventosCadastroConfiguracao_Acoes(
  Sender: TObject);
begin
  inherited;
  ...
  begin
    ...
    begin
      AddValorInicial('PRODUTO', 'TEM_USOECONSUMO', nil,  'S');
      AddValorInicial(FNomeTabela , 'CODPRODUTO_TIPO', cbxCodProduto_Tipo, BuscaCodigoTipoPelaClassificacao('USOECONSUMO'));
    end;
  end;
end;

initialization
  RegisterClass(TFrmProduto_UsoEConsumo_Frame);
  RegisterFrameCad(Tag_Produto_UsoEConsumo, TFrmProduto_UsoEConsumo_Frame);
